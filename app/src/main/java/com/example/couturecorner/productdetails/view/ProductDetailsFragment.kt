...
class ProductDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
       ...
       /* currencyViewModel.selectedCurrency.observe(viewLifecycleOwner) { currency ->
            price?.let { priceValue ->
                lifecycleScope.launch {
                    currencyViewModel.convertCurrency("EGP", currency, priceValue.toDouble()) { convertedPrice ->
                        if (convertedPrice != null) {
                            updatePriceDisplay(convertedPrice.toString())
                        } else {
                            Log.e("CurrencyConversion", "Currency conversion failed")
                        }
                    }
                }
            }
    ...
    private fun updatePriceDisplay(price: String) {
        binding.priceValue.text = "${price} " // Display the updated price
    }
